...
TRACEAGENT1=-javaagent:/home/service/tools/framework-trace-agent-6.2.0-SNAPSHOT.jar
case "`uname`" in
    Linux)
...
java $TRACEAGENT1 $JACOCO_OPTS $JAVA_OPTS $MEM_OPTS $OOM_OPTS $GC_OPTS -classpath 'conf:lib/*:.:/home/service/tools/trace-agent.properties' -jar lib/jacococoverage-0.0.1-SNAPSHOT.jar 1>>logs/server.log 2>&1 &
